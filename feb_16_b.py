...
# Các tham số khảo sát vùng làm việc
xmin=-(rp+rb)
xmax=rp+rb
ymin=-(rp+rb)
ymax=rp+rb
...
zmax=rp+rb
step=0.005
xstep=int((xmax-xmin)/step+1)
ystep=int((ymax-ymin)/step+1)
zstep=int((zmax-zmin)/step+1)

# Các biến lưu kết quả
workspace=[]

# Khảo sát vùng làm việc góc hướng hằng
t=0
for tz in range(0,zstep):
    for ty in range(0,ystep):
        for tx in range(0,xstep):
            t+=1
            pb0=[round(xmin+tx*step,8),round(ymin+ty*step,8),round(zmin+tz*step,8)]
            if (checkin(pb0,ax,ay,az)==1):
                workspace.append(pb0)
...
for i in range(0, len(workspace)):
    ax.plot3D(workspace[i][0],workspace[i][1],workspace[i][2],".")
plt.title("Vùng làm việc của Robot song song 6-RSS")
plt.show()
print(len(workspace))
